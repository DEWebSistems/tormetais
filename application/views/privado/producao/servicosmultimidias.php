...
<script type="text/javascript">
    ...
    function imagesServicosOnClick(elementImage, arquivoMultimidiaId, imagemPrincipal)
    {
...
?>
<h3 class="page-header-form-list">Dados do Serviço</h3>
...
<div style="margin-left: 10px; margin-right: 10px;">
    <?php echo form_open_multipart('privado/servicos/multimidias/' . $dadosServico['id'], array('class' => 'form-horizontal', 'role' => 'form')); ?>
        <input id="ihServicoId" name="ihServicoId" type="hidden" value="<?php echo $dadosServico['id']; ?>"/>
    ...
    ?>
    <div>
        Nenhuma imagem deste serviço foi cadastrada
    </div>
    <?php
        }
        else
        {
            foreach($dadosImagens as $dadosImagem)
            {
                ...
                {
                    echo 'class="img-rounded mainImage" onclick="imagesServicosOnClick(this, ' . $dadosImagem['arquivomultimidiaid'] .  ', true);"/>';
                }
                else
                {
                    echo 'class="img-rounded" onclick="imagesServicosOnClick(this, ' . $dadosImagem['arquivomultimidiaid'] .  ', false);"/>';
                }
            }
        }
    ?>
...
<div style="margin-left: 10px; margin-right: 10px;">
    <form action="<?php echo site_url('privado/servicos/multimidias/' . $dadosServico['id']); ?>" method="post" role="form" class="form-horizontal">
    ...
    <br/>
    <form action="<?php echo site_url('privado/servicos/multimidias/' . $dadosServico['id']); ?>" method="post">
    <?php
        if(empty($dadosVideos))
        {
    ?>
    <div>
        Nenhum vídeo deste serviço foi cadastrado
    </div>
    <?php
        }
        else
...
<br/>
<div style="margin-left: 10px; margin-right: 10px;">
    <a href="<?php echo site_url('privado/servicos'); ?>" class="btn btn-default"><span class="glyphicon glyphicon-arrow-left"></span>&nbsp;Voltar</a>
</div>
<br/>
