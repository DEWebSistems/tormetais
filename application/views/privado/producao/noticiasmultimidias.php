...
<script type="text/javascript">
    ...
    function imagesNoticiasOnClick(elementImage, arquivoMultimidiaId, imagemPrincipal)
    {
...
<h3 class="page-header-form-list">Dados da Notícia</h3>
    ...
    <table>
        <tbody>
            <tr>
                ...
                <td><?php echo $dadosNoticia['nome']; ?></td>
            </tr>
        </tbody>
    </table>
</div>
...
<div style="margin-left: 10px; margin-right: 10px;">
    <?php echo form_open_multipart('privado/noticias/multimidias/' . $dadosNoticia['id'], array('class' => 'form-horizontal', 'role' => 'form')); ?>
        <input id="ihNoticiaId" name="ihNoticiaId" type="hidden" value="<?php echo $dadosNoticia['id']; ?>"/>
    ...
    ?>
    <div>
        Nenhuma imagem desta notícia foi cadastrada
    </div>
    <?php
        }
        else
        {
            foreach($dadosImagens as $dadosImagem)
            {
                ...
                {
                    echo 'class="img-rounded mainImage" onclick="imagesNoticiasOnClick(this, ' . $dadosImagem['arquivomultimidiaid'] .  ', true);"/>';
                }
                else
                {
                    echo 'class="img-rounded" onclick="imagesNoticiasOnClick(this, ' . $dadosImagem['arquivomultimidiaid'] .  ', false);"/>';
                }
            }
        }
    ?>
...
<div style="margin-left: 10px; margin-right: 10px;">
    <form action="<?php echo site_url('privado/noticias/multimidias/' . $dadosNoticia['id']); ?>" method="post" role="form" class="form-horizontal">
    ...
    <br/>
    <form action="<?php echo site_url('privado/noticias/multimidias/' . $dadosNoticia['id']); ?>" method="post">
    <?php
        if(empty($dadosVideos))
        {
    ?>
    <div>
        Nenhum vídeo desta notícia foi cadastrado
    </div>
    <?php
        }
        else
...
<br/>
<div style="margin-left: 10px; margin-right: 10px;">
    <a href="<?php echo site_url('privado/noticias'); ?>" class="btn btn-default"><span class="glyphicon glyphicon-arrow-left"></span>&nbsp;Voltar</a>
</div>
<br/>
