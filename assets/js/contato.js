function validateBeforeSubmitRecord()
{
    var isErrors;
    var messagesHTML;
    isErrors = false;
    messagesHTML = '';
    messagesHTML += '<div class="alert alert-warning">';
    messagesHTML += '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>';
    messagesHTML += '<h4>Foram encontradas as seguintes inconsistências:</h4>';
    if(allTrim($('#itNome').val()) === '')
    {
        if(isErrors === false)
        {
            $('#itNome').focus();
        }
        isErrors = true;
        messagesHTML += '<br/>O campo "Nome" é de preenchimento obrigatório.';
    }
   
    if(allTrim($('#itTelefone').val()) === '')
    {
        if(isErrors === false)
        {
            $('#itTelefone').focus();
        }
        isErrors = true;
        messagesHTML += '<br/>O campo "Telefone" é de preenchimento obrigatório.';
    }
    
    if(allTrim($('#itEmailOrigem').val()) === '')
    {
        if(isErrors === false)
        {
            $('#itEmailOrigem').focus();
        }
        isErrors = true;
        messagesHTML += '<br/>O campo "E-mail" é de preenchimento obrigatório.';
    }
    if(allTrim($('#itAssunto').val()) === '')
    {
        if(isErrors === false)
        {
            $('#itAssunto').focus();
        }
        isErrors = true;
        messagesHTML += '<br/>O campo "Assunto" é de preenchimento obrigatório.';
    }
    if(allTrim($('#taMensagem').val()) === '')
    {
        if(isErrors === false)
        {
            $('#taMensagem').focus();
        }
        isErrors = true;
        messagesHTML += '<br/>O campo "Mensagem" é de preenchimento obrigatório.';
    }
    
    messagesHTML += '</div>';
    if(isErrors === true)
    {
        $('#divResultsValidations').html(messagesHTML);
        window.scrollTo(0, 0);
        return false;
    }
    else if(isErrors === false)
    {
        $('#divResultsValidations').html('');
        window.scrollTo(0, 0);
        return true;
    }
}